...
class Task(Base):
    ...
    start_time = Column(DateTime(timezone=True))    # <-- חדש: זמן התחלה אמיתי
    end_time = Column(DateTime(timezone=True))      # זמן סיום אמיתי
    ...
    def to_dict(self):
        ...
        if self.start_time:
            try:
                start_str = self.start_time.astimezone(TZ).strftime("%H:%M:%S")
            except:
                start_str = "-"
        else:
            start_str = "-"
...
def recompute_chain():
    """מעדכן משימות שרצות ומפעיל את הבאה אם נגמרה"""
