...
class Task(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "real_end": self.end_time.astimezone(TZ).strftime("%H:%M:%S")
...
# ==========================================
# ❗❗ פונקציה חדשה — Pending + Reset ❗❗
# ==========================================

@app.route("/pending_reset/<int:tid>", methods=["POST"])
def pending_reset(tid):
    with session_scope() as s:
        t = s.get(Task, tid)
        if t:
            t.status = "pending"
...
def update_task(tid):
    ...
    with session_scope() as s:
        ...
        if t:
            ...
            if t.status == "running":
                t.remaining = dur
                t.end_time = now() + timedelta(seconds=dur)
